...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.companies.update", [$company->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="map_link">{{ trans('cruds.company.fields.map_link') }}</label>
                <textarea class="form-control {{ $errors->has('map_link') ? 'is-invalid' : '' }}" name="map_link" id="map_link">{{ old('map_link', $company->map_link) }}</textarea>
                @if($errors->has('map_link'))
                    <div class="invalid-feedback">
                        {{ $errors->first('map_link') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.company.fields.map_link_helper') }}</span>
            </div>
            <div class="form-group">
