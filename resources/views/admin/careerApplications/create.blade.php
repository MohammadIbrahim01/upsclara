...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.careerApplication.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.career-applications.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('qualifications') ? 'is-invalid' : '' }}" name="qualifications" id="qualifications">{{ old('qualifications') }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('job_opening') ? 'is-invalid' : '' }}" name="job_opening_id" id="job_opening_id">
                    @foreach($job_openings as $id => $entry)
                        <option value="{{ $id }}" {{ old('job_opening_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
