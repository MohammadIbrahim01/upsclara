...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.blogs.update", [$blog->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="heading">{{ trans('cruds.blog.fields.heading') }}</label>
                <input class="form-control {{ $errors->has('heading') ? 'is-invalid' : '' }}" type="text" name="heading" id="heading" value="{{ old('heading', $blog->heading) }}">
                @if($errors->has('heading'))
                    <div class="invalid-feedback">
                        {{ $errors->first('heading') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.blog.fields.heading_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="short_description">{{ trans('cruds.blog.fields.short_description') }}</label>
                <textarea class="form-control {{ $errors->has('short_description') ? 'is-invalid' : '' }}" name="short_description" id="short_description">{{ old('short_description', $blog->short_description) }}</textarea>
            ...
            <div class="form-group">
                <label for="featured_image_caption">{{ trans('cruds.blog.fields.featured_image_caption') }}</label>
                <input class="form-control {{ $errors->has('featured_image_caption') ? 'is-invalid' : '' }}" type="text" name="featured_image_caption" id="featured_image_caption" value="{{ old('featured_image_caption', $blog->featured_image_caption) }}">
                @if($errors->has('featured_image_caption'))
                    <div class="invalid-feedback">
                        {{ $errors->first('featured_image_caption') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.blog.fields.featured_image_caption_helper') }}</span>
            </div>
            <div class="form-group">
                <label>{{ trans('cruds.blog.fields.type') }}</label>
                @foreach(App\Models\Blog::TYPE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('type') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="type_{{ $key }}" name="type" value="{{ $key }}" {{ old('type', $blog->type) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('publish_date') ? 'is-invalid' : '' }}" type="text" name="publish_date" id="publish_date" value="{{ old('publish_date', $blog->publish_date) }}">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('active') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="active" id="active" value="1" {{ $blog->active || old('active', 0) === 1 ? 'checked' : '' }}>
                    <label class="form-check-label" for="active">{{ trans('cruds.blog.fields.active') }}</label>
                </div>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.blog.fields.active_helper') }}</span>
            </div>
            <div class="form-group">
