...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.courseFaq.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.course-faqs.update", [$courseFaq->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('course') ? 'is-invalid' : '' }}" name="course_id" id="course_id" required>
                    @foreach($courses as $id => $entry)
                        <option value="{{ $id }}" {{ (old('course_id') ? old('course_id') : $courseFaq->course->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('question') ? 'is-invalid' : '' }}" type="text" name="question" id="question" value="{{ old('question', $courseFaq->question) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('answer') ? 'is-invalid' : '' }}" name="answer" id="answer">{{ old('answer', $courseFaq->answer) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('sequence') ? 'is-invalid' : '' }}" type="number" name="sequence" id="sequence" value="{{ old('sequence', $courseFaq->sequence) }}" step="1">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('active') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="active" id="active" value="1" {{ $courseFaq->active || old('active', 0) === 1 ? 'checked' : '' }}>
